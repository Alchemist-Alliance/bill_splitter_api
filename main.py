...
def create_bill():
    ...
    try:
        data = request.get_json()
        event = fetch_event(event_key=data[EVENT_KEY])
        check_event_before_creating_bill(
            event=event, drawees=data[DRAWEES], payees=data[PAYEES])
        bill = create_bill_in_database(data)
        add_bill_in_event(drawees=bill[DRAWEES], payees=bill[PAYEES],
                          event=event, amount=bill[AMOUNT],  bill_key=bill[KEY])
        update_event(event=event)
...
def delete_bill():
    ...
    try:
        ...
        remove_bill_from_event(
            drawees=bill[DRAWEES], payees=bill[PAYEES], event=event, amount=bill[AMOUNT],  bill_key=bill[KEY])
        update_event(event=event)
